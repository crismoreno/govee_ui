...
const deviceScenesContainer = document.querySelector('.device-scenes-container');

const displayDeviceScenes = (scenes) => {
	scenes.forEach((scene) => {
		const sceneDiv = document.createElement('div');
		sceneDiv.classList.add('scene-item', 'm-1', 'p-2', 'border', 'rounded', 'col-auto', 'text-center');
...
const getDevicesScenes = async ({apiKey, device: {sku, device}}) => {
	const devicesScenesResponse = await getDeviceScenes({apiKey, deviceMac: device, deviceSku: sku});
